...
class Home extends React.Component {
  ...
  componentDidMount = () => {
    ...
    let menu = new Promise((resolve, reject) => {
      axios.get('http://127.0.0.1:9091/mock/homes/menu').then((data)=>{
        resolve(data.data.list);
      });
    })
    //首页房源资讯
    let info = new Promise((resolve, reject) => {
      axios.get('http://127.0.0.1:9091/mock/homes/info').then((data)=>{
        resolve(data.data.list);
      });
    })
    //首页问答
    let faq = new Promise((resolve, reject) => {
      axios.get('http://127.0.0.1:9091/mock/homes/faq').then((data)=>{
        resolve(data.data.list);
      });
    })
    //首页最新房源
    let house = new Promise((resolve, reject) => {
      axios.get('http://127.0.0.1:9091/mock/homes/house').then((data)=>{
        resolve(data.data.list);
      });
    })
